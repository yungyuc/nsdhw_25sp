...
public:
    ...
    Matrix(size_t nrow, size_t ncol)
      : m_nrow(nrow), m_ncol(ncol), m_buffer(m_alloc)
    {
...
public:
    ...
    size_t bytes() const { return m_alloc.counter.bytes(); }
    size_t allocated() const { return m_alloc.counter.allocated(); }
    size_t deallocated() const { return m_alloc.counter.deallocated(); }

    size_t m_nrow = 0;
    size_t m_ncol = 0;
    MyAllocator<double> m_alloc;
