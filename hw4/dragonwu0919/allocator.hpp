...
{
    std::atomic_size_t allocated{0};   // 初始化為 0
    std::atomic_size_t deallocated{0}; // 初始化為 0
    std::atomic_size_t refcount{0};    // 初始化為 0
...
inline bool operator==(ByteCounter const & a, ByteCounter const & b)
{
...
static ByteCounter global_counter ;
...
struct MyAllocator
{
    ...
    static size_t getbytes()  { return global_counter.bytes(); }
    static size_t getallocated()  { return global_counter.allocated(); }
    static size_t getdeallocated()  { return global_counter.deallocated(); }

}; /* end struct MyAllocator */

extern template struct MyAllocator<double>;
