...
public:
    ...
    {
        m_buffer.shrink_to_fit(); // capacity to size
    }
...
size_t bytes() {
    return MyAllocator<double>::get_current();
}

size_t allocated() {
    return MyAllocator<double>::get_allocated();
}

size_t deallocated() {
    return MyAllocator<double>::get_deallocated();
}
